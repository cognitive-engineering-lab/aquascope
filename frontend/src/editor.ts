...
class TextIco implements Icon {
    ...
    getAuxiliary(): Array<Range<Decoration>> {
        ...
        let loan_deco = Decoration
            .mark({ class: "cm-loan" })
            .range(this.on_hover.loan_location.char_start, this.on_hover.loan_location.char_end);
    ...
    toDom(): HTMLElement {
        ...
        tt.addEventListener('mouseenter', (_) => {
            let elems = document.querySelectorAll(`.${this.getTag()}`)! as HTMLElement[];
    ...
    return StateField.define<DecorationSet>({
        ...
        update(points, transactions) {
            ...
            for (let e of transactions.effects) {
                if (e.is(ty)) {
                    return RangeSet.of(e.value.flatMap(([ico_l, ico_m, ico_r, from]) => {
                        let main_deco = cons([ico_l, ico_m, ico_r]).range(from);
                        return [main_deco,
                                ...ico_l.getAuxiliary(),
                                ...ico_m.getAuxiliary(),
                                ...ico_r.getAuxiliary()].sort((r1, r2) => r1.from - r2.from);
                    }), true);
                }
            }
...
let make_decoration_with_text_ico = <I extends TextIco>(icos: Array<I>): Decoration => {
    let fst = icos[0];
    let snd = icos[1];
    let trd = icos[2];
