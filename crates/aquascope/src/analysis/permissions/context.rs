...
impl<'a, 'tcx> PermissionsCtxt<'a, 'tcx> {
    ...
    pub fn moveable_path_to_path(&self, mp: MoveablePath) -> Path {
        let path = self.place_to_path(&self.move_data.move_paths[mp].place);
        log::debug!(
            "Place {:?} MP {:?} to P {:?}",
            self.path_to_place(path),
            mp,
            path
        );
        path
    ...
    pub fn is_declared_readonly(&self, place: &Place<'tcx>) -> bool {
        self.body_with_facts.body.local_decls[place.local].mutability != Mutability::Mut
    }
}
