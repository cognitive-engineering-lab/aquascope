...
use rustc_middle::mir::{
  self, graphviz_safe_def_name, BasicBlock, Body, Location,
};
use rustc_mir_dataflow::{
  fmt::{DebugDiffWithAdapter, DebugWithAdapter, DebugWithContext},
  Analysis, CallReturnPlaces, Direction, Results, ResultsRefCursor,
  ResultsVisitor,
};
  ...
  A::Domain: DebugWithContext<A>,
{
  ...
  fn edge_label(&self, e: &Self::Edge) -> dot::LabelText<'_> {
    let label =
      &self.body[e.source].terminator().kind.fmt_successor_labels()[e.index];
  ...
  A::Domain: DebugWithContext<A>,
{
  ...
  fn write_node_label(
    ...
    match self.style {
      ...
      OutputStyle::BeforeAndAfter => self
        .write_block_header_with_state_columns(w, block, &[
          "BEFORE", "AFTER",
        ])?,
    }
  ...
  fn write_block_header_simple(
    ...
    write!(
      ...
      concat!(
        "<tr>",
        r#"<td colspan="3" sides="tl">bb{block_id}</td>"#,
        "</tr>",
      ),
  ...
  fn write_block_header_with_state_columns(
    ...
    write!(
      w,
      concat!("<tr>", r#"<td colspan="2" {fmt}>MIR</td>"#,),
      fmt = fmt,
    )?;
  ...
  fn write_statements_and_terminator(
    ...
    let diffs =
      StateDiffCollector::run(body, block, self.results.results(), self.style);
  ...
  fn write_row<W: io::Write>(
    ...
    let valign = if mir.starts_with("(on ") && mir != "(on entry)" {
      "bottom"
    } else {
      "top"
    };
  ...
  A::Domain: DebugWithContext<A>,
{
  ...
  fn visit_statement_after_primary_effect(
    ...
    self
      .after
      .push(diff_pretty(state, &self.prev_state, self.analysis));
    self.prev_state.clone_from(state)
  }
  ...
  T: DebugWithContext<C>,
{
  ...
  let html_diff =
    re.replace_all(&raw_diff, |captures: &regex::Captures<'_>| {
